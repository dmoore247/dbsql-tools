...
class PrerequisiteValidator:
    ...
    def validate_permission(self):
        ...
        rows = spark.sql(f"select table_catalog, table_schema, table_name from system.information_schema.table_privileges where table_catalog = 'system' AND grantee = '{USERNAME}';").collect()
        ...
        if set(REQUIRED_SYSTEM_TABLES).issubset(set(granted_tables)):
            ...
            result = self.ValidationResult(requirement='permission', is_pass=False, error=f"You do not have permission to access the necessary system tables {missing_tables}. Contact metastore admin for permission")
    ...
    def validate_warehouse(self):
        ...
        if not acceptable_warehouses:
            result = self.ValidationResult(requirement='warehouse', is_pass=False, error="No acceptable warehouses found. Please have a Medium or Large warehouse available")
        else:
